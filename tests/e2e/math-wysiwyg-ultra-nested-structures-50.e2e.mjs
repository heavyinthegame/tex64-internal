...
const log = (message) => {
  console.log(`[math-wysiwyg-ultra-nested-structures-50-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-ultra-nested-structures-50-")
  );
...
const complexAtoms = (i) => ({
  lhs1: `(∂/∂t)∫(0→1)(u${i}(x,t)^2+|∇u${i}|^2)dx+Σ(k=1→n)(-1)^k/(k+α${i})`,
  rhs1: `Γ(α${i}+β${i})/(Γ(α${i})Γ(β${i}))+det(A${i}+λI)-tr(A${i})`,
  lhs2: `∇·(ρ${i}v${i})+∂ρ${i}/∂t+Δu${i}`,
  rhs2: `argmin(θ${i})(L(θ${i})+β||θ${i}||^2)+E[X${i}|Y${i}]`,
  cond1: `x>${i}`,
  cond2: `x≤${i}`,
  tail1: `∫(0→∞)e^(-s t)f${i}(t)dt`,
  tail2: `Σ(j=1→m)aj${i}/(1+bj${i}^2)`,
  tail3: `lim(r→∞)(1+1/r)^r+log(1+r${i}^2)`,
  plus1: `∂^2u${i}/∂x^2+∂^2u${i}/∂y^2`,
  plus2: `∇×F${i}+∇·F${i}+∫(Ω)g${i}(x)dx`,
});

const buildNestedInsertActions = (spec, i) => {
  const atom = complexAtoms(i);
  if (spec.kind === "cases") {
    return [
      { suggest: spec.suggest, pickIndex: spec.pickIndex ?? 0 },
      atom.lhs1,
      { key: "Tab" },
      atom.cond1,
      { key: "Tab" },
...
const buildMatrixUltraCase = (outer, inner, operation, i) => {
  const atom = complexAtoms(i);
  const marker = operation === "row" ? `r${i}` : `c${i}`;
  return {
    name: `${outer.token} ultra nested ${inner.id} ${operation} #${i}`,
    input: `${outer.token}-${inner.id}-${operation}-${i}`,
    actions: [
      { suggest: outer.token, pickIndex: 0 },
      ...buildNestedInsertActions(inner, i),
      { key: "Tab" },
      atom.tail1,
      { key: "Tab" },
      atom.tail2,
      { key: "Tab" },
      atom.tail3,
      { key: operation === "row" ? "Enter" : addColumnShortcut },
      `${marker}A=${atom.plus1}`,
      { key: "Tab" },
      `${marker}B=${atom.plus2}`,
    ],
    ...
    minLineBreaks: operation === "row" ? 3 : 2,
    minAmpersands: operation === "row" ? 4 : 5,
    snippetTokens: [outer.begin, outer.end, inner.begin, [`${marker}A`]],
  };
};

const buildAlignedUltraCase = (leftSpec, rightSpec, i) => {
  const atom = complexAtoms(i);
  return {
    name: `aligned ultra nested ${leftSpec.id}+${rightSpec.id} #${i}`,
    input: `aligned-${leftSpec.id}-${rightSpec.id}-${i}`,
    actions: [
      { suggest: "aligned", pickIndex: 0 },
      ...buildNestedInsertActions(leftSpec, i),
      { key: "Tab" },
      atom.tail1,
      { key: "Tab" },
      ...buildNestedInsertActions(rightSpec, i + 50),
      { key: "Tab" },
      `${atom.tail2}+${atom.plus1}`,
    ],
    checks: [
      ["\\begin{aligned}"],
      ["\\end{aligned}"],
    ...
    minLineBreaks: 2,
    minAmpersands: 4,
    snippetTokens: [["\\begin{aligned}"], ["\\end{aligned}"], leftSpec.begin, rightSpec.begin],
  };
};

const ADVERSARIAL_NESTED_MATRIX_CASES = [];
let caseCounter = 1;

for (const outer of MATRIX_OUTER_ENVS) {
  for (const inner of MATRIX_INNER_SPECS) {
    for (const operation of ["row", "col"]) {
      ADVERSARIAL_NESTED_MATRIX_CASES.push(
        buildMatrixUltraCase(outer, inner, operation, caseCounter)
      );
      caseCounter += 1;
    }
  }
...
for (let idx = 0; ADVERSARIAL_NESTED_MATRIX_CASES.length < 50; idx += 1) {
  const left = ALIGNED_INNER_SPECS[idx % ALIGNED_INNER_SPECS.length];
  const right = ALIGNED_INNER_SPECS[(idx + 1) % ALIGNED_INNER_SPECS.length];
  ADVERSARIAL_NESTED_MATRIX_CASES.push(
    buildAlignedUltraCase(left, right, caseCounter)
  );
  caseCounter += 1;
}

ADVERSARIAL_NESTED_MATRIX_CASES.splice(50);
const runOnce = async () => {
  const from = Math.max(1, Number.parseInt(process.env.E2E_ULTRA_FROM ?? "1", 10) || 1);
  const toInput =
    Number.parseInt(process.env.E2E_ULTRA_TO ?? String(ADVERSARIAL_NESTED_MATRIX_CASES.length), 10) ||
    ADVERSARIAL_NESTED_MATRIX_CASES.length;
  const to = Math.min(ADVERSARIAL_NESTED_MATRIX_CASES.length, Math.max(from, toInput));
  const formulas = ADVERSARIAL_NESTED_MATRIX_CASES.slice(from - 1, to);
  assert.equal(
    ADVERSARIAL_NESTED_MATRIX_CASES.length,
    50,
    "ultra nested structures dataset must contain exactly 50 entries"
  );
  ...
  try {
    ...
    log(`formula range ${from}-${to} / total ${ADVERSARIAL_NESTED_MATRIX_CASES.length}`);
    ...
    for (const [index, formula] of formulas.entries()) {
      ...
      const label = `[${seq}/${ADVERSARIAL_NESTED_MATRIX_CASES.length}] ${formula.name}`;
    ...
    log("math-wysiwyg ultra nested structures 50 e2e passed");
  } finally {
    if (electronApp) {
      try {
        await Promise.race([
...
const run = async () => {
  ...
  throw lastError ?? new Error("math-wysiwyg ultra nested structures 50 e2e failed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-ultra-nested-structures-50-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
