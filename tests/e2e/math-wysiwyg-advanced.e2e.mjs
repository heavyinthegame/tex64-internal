...
const log = (message) => {
  console.log(`[math-wysiwyg-advanced-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-advanced-"));
...
const run = async () => {
  await runCase("[1/10] lim with bounds", async (page) => {
    await applySuggestionByTyping(page, "lim", { pickIndex: 1 });
    await page.keyboard.type("x", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.type("a", { delay: typeDelayMs });
    await waitForExactLatex(page, "\\lim_{x \\to a}", "lim");
    await assertRenderedOutput(page, {
      label: "lim",
      mustContain: ["x", "a"],
      mustContainAny: [["lim", "lim"]],
      mustNotContain: ["placeholder"],
    });
  });

  await runCase("[2/10] argmax operator", async (page) => {
    await applySuggestionByTyping(page, "argmax", { pickIndex: 0 });
    await waitForExactLatex(page, "\\operatorname*{arg\\,max}", "argmax");
    await assertRenderedOutput(page, {
      label: "argmax",
      mustContainAny: [["arg", "max", "argmax"]],
      mustNotContain: ["placeholder"],
    });
  });

  await runCase("[3/10] piecewise cases", async (page) => {
    await applySuggestionByTyping(page, "cases", { pickIndex: 0 });
    await page.keyboard.type("x", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.type("x>0", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.type("-x", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.type("x<0", { delay: typeDelayMs });
    await waitForExactLatex(
      page,
    ...
    await assertRenderedOutput(page, {
      label: "cases",
      mustContain: ["x", ">0", "<0"],
      mustNotContain: ["cases", "placeholder"],
    });
  });

  await runCase("[4/10] pmatrix template", async (page) => {
    await applySuggestionByTyping(page, "pmatrix", { pickIndex: 0 });
    await page.keyboard.type("a", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    ...
    await page.keyboard.press("Tab");
    await page.keyboard.type("d", { delay: typeDelayMs });
    await waitForExactLatex(
      page,
    ...
    await assertRenderedOutput(page, {
      label: "pmatrix",
      mustContain: ["a", "b", "c", "d"],
      mustNotContain: ["pmatrix", "placeholder"],
    });
  });

  await runCase("[5/10] set-builder template", async (page) => {
    await applySuggestionByTyping(page, "set", { pickIndex: 0 });
    await page.keyboard.type("x", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.type("x>0", { delay: typeDelayMs });
    await waitForExactLatex(page, "\\left\\{x \\mid x>0\\right\\}", "set");
    await assertRenderedOutput(page, {
      label: "set",
      mustContain: ["x", ">0"],
      mustNotContain: ["placeholder"],
  ...
  await runCase("[6/10] vector accent", async (page) => {
    await applySuggestionByTyping(page, "vec", { pickIndex: 0 });
    await page.keyboard.type("x", { delay: typeDelayMs });
    await waitForExactLatex(page, "\\vec{x}", "vec");
    await assertRenderedOutput(page, {
      label: "vec",
      mustContain: ["x"],
      mustContainAny: [["→", "⃗"]],
  ...
  await runCase("[7/11] blackboard bold template", async (page) => {
    await applySuggestionByTyping(page, "bb", { pickIndex: 0 });
    await waitForExactLatex(page, "\\mathbb{R}", "bb");
    await assertRenderedOutput(page, {
      label: "bb",
      mustContainAny: [["R"]],
      mustNotContain: ["placeholder"],
  ...
  await runCase("[8/11] blackboard bold set", async (page) => {
    await applySuggestionByTyping(page, "real", { pickIndex: 0 });
    await waitForExactLatex(page, "\\mathbb{R}", "real");
    await assertRenderedOutput(page, {
      label: "real",
      mustContainAny: [["R"]],
      mustNotContain: ["placeholder"],
  ...
  await runCase("[9/11] auto operator d/dx", async (page) => {
    await focusMathField(page);
    await page.keyboard.type("d/dx", { delay: typeDelayMs });
    await page.keyboard.type("y", { delay: typeDelayMs });
    await page.keyboard.press("Tab");
    await page.keyboard.press("Tab");
    await page.keyboard.type("x", { delay: typeDelayMs });
    await waitForExactLatex(page, "\\frac{\\mathrm{dy}}{\\mathrm{dx}}", "d/dx");
    await assertRenderedOutput(page, {
      label: "d/dx",
      mustContain: ["y", "x"],
      mustContainAny: [["d"]],
      mustNotContain: ["d/dx", "placeholder"],
    });
  });

  await runCase("[10/11] partial symbol", async (page) => {
    await applySuggestionByTyping(page, "partial", { pickIndex: 0 });
    await waitForExactLatex(page, "\\partial", "partial");
    await assertRenderedOutput(page, {
      label: "partial",
      mustContainAny: [["∂"]],
      mustNotContain: ["placeholder"],
  ...
  log("math-wysiwyg advanced e2e passed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-advanced-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
