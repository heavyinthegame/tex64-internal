...
const log = (message) => {
  console.log(`[math-wysiwyg-aligned-regression-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-aligned-"));
...
const run = async () => {
  await runCase("[1/2] aligned suggestion typing should keep render stable", async (page) => {
  ...
  await runCase("[2/2] aligned placeholder navigation keeps row structure", async (page) => {
    await applySuggestionByTyping(page, "aligned", 0);

    await typeWithRenderChecks(page, "x+1", "edit-after-insert: lhs row1");
    await page.keyboard.press("Tab");
    await typeWithRenderChecks(page, "4", "edit-after-insert: rhs row1");
    await page.keyboard.press("Tab");
    await typeWithRenderChecks(page, "2x+y", "edit-after-insert: lhs row2");
    await page.keyboard.press("Tab");
    await typeWithRenderChecks(page, "5", "edit-after-insert: rhs row2");
    await assertRenderHealthy(page, "edit-after-insert: filled", { allowPlaceholder: false });

    await waitForLatexContainsAll(
      page,
      ["\\begin{aligned}", "\\end{aligned}", "x+1&=4", "2x+y&=5", "\\\\"],
      "edit-after-insert: final latex"
    );
    await assertRenderHealthy(page, "edit-after-insert: final render", { allowPlaceholder: false });
  });

  log("math-wysiwyg aligned regression e2e passed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-aligned-regression-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
