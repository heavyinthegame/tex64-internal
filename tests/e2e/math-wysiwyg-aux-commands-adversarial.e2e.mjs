...
const log = (message) => {
  console.log(`[math-wysiwyg-aux-commands-adversarial-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-aux-commands-adversarial-")
  );
...
const finalizeMathFieldNormalization = async (page) => {
...
const run = async () => {
  await runCase("[1/4] label with punctuation stays stable in aligned", async (page) => {
    await applySuggestionByTyping(page, "aligned", 0);
    await applySuggestionByTyping(page, "label", 0);
    await typeWithoutRenderChecks(page, "{sec/intro+alpha@v1}");
    await finalizeMathFieldNormalization(page);

    await waitForLatexContainsAll(
      page,
      ["\\begin{aligned}", "\\end{aligned}", "\\txlbl{sec/intro+alpha@v1}"],
      "label punctuation"
    );

    await assertRenderHealthy(page, "label punctuation render", { allowPlaceholder: true });
  });

  await runCase("[2/4] nonumber insertion remains stable", async (page) => {
    await applySuggestionByTyping(page, "aligned", 0);
    ...
    await assertRenderHealthy(page, "nonumber stability render", { allowPlaceholder: true });
  });

  await runCase("[3/4] intertext insertion remains stable", async (page) => {
    await applySuggestionByTyping(page, "aligned", 0);
    ...
    await assertRenderHealthy(page, "intertext insertion render", { allowPlaceholder: true });
  });

  await runCase("[4/4] shortintertext and spaced tag star normalize", async (page) => {
    await applySuggestionByTyping(page, "aligned", 0);
    await applySuggestionByTyping(page, "tag", 1);
    await typeWithoutRenderChecks(page, "A-1");
    await finalizeMathFieldNormalization(page);
    ...
    await assertRenderHealthy(page, "spaced tag star render", {
      allowPlaceholder: true,
    });
  });

  log("math-wysiwyg aux commands adversarial e2e passed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-aux-commands-adversarial-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
