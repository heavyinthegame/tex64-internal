...
const log = (message) => {
  console.log(`[math-wysiwyg-environment-entangled-50-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-environment-entangled-50-")
  );
...
const COMPLEX_TEMPLATE_SETS = [
  {
    id: "spectral-balance",
    mode: "inline",
    cells: ["Σ(k=1→n)ak", "∫(0→1)x^2dx", "exp(iθ)+λ", "∂u/∂t+Δu"],
    checks: [["\\Sigma", "Σ"], ["\\int", "∫"], ["\\theta", "θ"], ["\\partial", "∂"]],
    snippetToken: ["\\Sigma", "Σ"],
  },
  {
    id: "vector-field-coupling",
    mode: "align",
    cells: ["(a+b)^2/(1+a^2)", "log(1+x^2)", "∇·F+ρ", "sin(ωt)+cos(ωt)"],
    checks: [["log"], ["\\nabla", "∇"], ["\\rho", "ρ"], ["\\omega", "ω"]],
    snippetToken: ["\\nabla", "∇"],
  },
  {
    id: "operator-asymptotic",
    mode: "gather",
    cells: ["det(A+λI)", "tr(B^TB)", "Σ(j=1→m)(1+μj)", "lim(r→∞)r^2"],
    checks: [["det"], ["tr("], ["\\Sigma", "Σ"], ["\\mu", "μ"], ["\\infty", "∞"]],
    snippetToken: ["det"],
  },
  {
    id: "contour-entropy",
    mode: "align",
    cells: ["Re(z)+Im(z)", "||x||^2+||y||^2", "argmax(p)H(p)", "∮C f(z)dz"],
    checks: [["Re(z)"], ["argmax"], ["\\oint", "∮"]],
    snippetToken: ["\\oint", "∮"],
  },
  {
    id: "probabilistic-gamma",
    mode: "inline",
    cells: ["E[X|Y=y]", "Var(X)+Cov(X,Y)", "Γ(α+1)", "βlogZ"],
    checks: [["E[X|Y=y]"], ["Var(X)"], ["\\Gamma", "Γ"], ["\\beta", "β"]],
...
const ENVIRONMENT_50_CASES = ENVIRONMENT_TEMPLATES.flatMap((env) =>
  COMPLEX_TEMPLATE_SETS.map((set, index) => {
...
const runOnce = async () => {
  const from = Math.max(1, Number.parseInt(process.env.E2E_ENV_ENTANGLED_FROM ?? "1", 10) || 1);
  const toInput =
    Number.parseInt(process.env.E2E_ENV_ENTANGLED_TO ?? String(ENVIRONMENT_50_CASES.length), 10) ||
    ENVIRONMENT_50_CASES.length;
  const to = Math.min(ENVIRONMENT_50_CASES.length, Math.max(from, toInput));
  const formulas = ENVIRONMENT_50_CASES.slice(from - 1, to);
  assert.equal(ENVIRONMENT_50_CASES.length, 50, "environment entangled dataset must contain 50 entries");
  ...
  try {
    ...
    log(`formula range ${from}-${to} / total ${ENVIRONMENT_50_CASES.length}`);
    ...
    for (const [index, formula] of formulas.entries()) {
      ...
      const label = `[${seq}/${ENVIRONMENT_50_CASES.length}] ${formula.name}`;
    ...
    log("math-wysiwyg environment entangled 50 e2e passed");
  } finally {
    if (electronApp) {
      try {
        await Promise.race([
...
const run = async () => {
  ...
  throw lastError ?? new Error("math-wysiwyg environment entangled 50 e2e failed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-environment-entangled-50-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
