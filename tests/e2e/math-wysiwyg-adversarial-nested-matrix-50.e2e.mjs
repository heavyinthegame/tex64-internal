...
const log = (message) => {
  console.log(`[math-wysiwyg-adversarial-nested-matrix-50-e2e ${now()}] ${message}`);
};
...
const createWorkspaceCopy = async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "tex64-e2e-math-wysiwyg-adversarial-nested-matrix-50-")
  );
...
const buildNestedActions = (nested, index) => {
  if (nested.id === "cases") {
    return [
      `x${index}^2+α${index}`,
      { key: "Tab" },
...
const buildAdversarialCase = (outer, nested, operation, index) => {
  const marker = operation === "row" ? `r${index}` : `c${index}`;
  return {
    name: `${outer.token} nested ${nested.id} ${operation} insertion #${index}`,
    input: `${outer.token}-${nested.id}-${operation}-${index}`,
    actions: [
      { suggest: outer.token, pickIndex: 0 },
      { suggest: nested.suggest, pickIndex: nested.pickIndex ?? 0 },
      ...buildNestedActions(nested, index),
      { key: "Tab" },
      `u${index}`,
      { key: "Tab" },
      `v${index}`,
    ...
    snippetTokens: [outer.begin, outer.end, nested.begin, [`${marker}A`]],
  };
};

const ADVERSARIAL_NESTED_MATRIX_CASES = [];
let caseCounter = 1;
outerLoop: for (const outer of OUTER_ENV_VARIANTS) {
  for (const nested of NESTED_STRUCTURES) {
    for (const operation of ["row", "col"]) {
      ADVERSARIAL_NESTED_MATRIX_CASES.push(
        buildAdversarialCase(outer, nested, operation, caseCounter)
      );
      caseCounter += 1;
      if (ADVERSARIAL_NESTED_MATRIX_CASES.length >= 50) {
        break outerLoop;
      }
    }
  }
...
const runOnce = async () => {
  const from = Math.max(1, Number.parseInt(process.env.E2E_ADVERSARIAL_FROM ?? "1", 10) || 1);
  const toInput =
    Number.parseInt(process.env.E2E_ADVERSARIAL_TO ?? String(ADVERSARIAL_NESTED_MATRIX_CASES.length), 10) ||
  ...
  assert.equal(
    ADVERSARIAL_NESTED_MATRIX_CASES.length,
    50,
    "adversarial nested matrix dataset must contain exactly 50 entries"
  );
  ...
  try {
    ...
    log("math-wysiwyg adversarial nested matrix 50 e2e passed");
  } finally {
    if (electronApp) {
      try {
        await Promise.race([
...
const run = async () => {
  ...
  throw lastError ?? new Error("math-wysiwyg adversarial nested matrix 50 e2e failed");
};

run().catch((error) => {
  console.error("[math-wysiwyg-adversarial-nested-matrix-50-e2e] FAILED");
  console.error(error);
  process.exitCode = 1;
});
