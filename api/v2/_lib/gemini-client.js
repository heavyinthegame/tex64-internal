...
export const buildUsageMetadata = (payload) => {
  ...
  const promptTokenCount = Math.max(
    ...
    Math.round(
      parseNumber(
        usage.promptTokenCount ?? usage.promptTokens ?? usage.inputTokenCount ?? usage.inputTokens ?? usage.input_tokens,
        0
      )
    )
  );
  const candidatesTokenCount = Math.max(
    ...
    Math.round(
      parseNumber(
        usage.candidatesTokenCount ?? usage.outputTokenCount ?? usage.outputTokens ?? usage.output_tokens,
        0
      )
    )
  );
  const rawTotal = Math.round(
    parseNumber(
      usage.totalTokenCount ?? usage.totalTokens ?? usage.quotaConsumedTokens ?? usage.total_tokens,
      Number.NaN
    )
  );
  ...
  return { promptTokenCount, candidatesTokenCount, totalTokenCount: Math.max(0, totalTokenCount) };
};

// Shared setup: validates API key, resolves endpoint and model name.
const resolveCall = (options) => {
  ...
  return { apiKey, endpointBase, model };
};

export const invokeGemini = async (requestPayload, options = {}) => {
  const { apiKey, endpointBase, model } = resolveCall(options);
  ...
  if (!upstreamResponse.ok) {
    const message =
      payload?.error?.message || payload?.message || `Gemini upstream returned HTTP ${upstreamResponse.status}.`;
    throw new ApiError("INTERNAL_ERROR", message, 502);
  }
  const responsePayload = asObject(payload) || {};
  return { raw: responsePayload, usageMetadata: buildUsageMetadata(responsePayload), model };
};

// Returns the raw streaming Response so the caller can forward SSE to the client.
export const invokeGeminiStream = async (requestPayload, options = {}) => {
  const { apiKey, endpointBase, model } = resolveCall(options);
  const url = `${endpointBase}/${encodeURIComponent(model)}:streamGenerateContent?key=${encodeURIComponent(apiKey)}&alt=sse`;
  let response;
  try {
