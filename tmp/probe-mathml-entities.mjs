...
const run = async () => {
  ...
  try {
    ...
    const out = await page.evaluate((arr) => {
      ...
      return arr.map((s) => {
        const m = ml.convertLatexToMathMl(`\\displaystyle ${s}`) || '';
        const entities = m.match(/&[^;\s]+;/g) || [];
        const bareAmp = /(^|[^&])&([^#a-zA-Z]|$)|<mi>&<\/mi>|<mo>&<\/mo>|<mtext>&<\/mtext>/.test(m);
        return { input: s.slice(0,90), entities: Array.from(new Set(entities)), bareAmp, hasNbsp: m.includes('&nbsp;') };
      });
    }, formulas);
