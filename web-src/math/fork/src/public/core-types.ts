...
|`"latex"`    | The string is a LaTeX fragment|
  ...
  */
  ...
  /**
   * If `true`, preserve undo history across consecutive `setValue()` calls.
   * This is particularly useful when an integration synchronizes external state
   * back into the mathfield in small increments.
   */
  preserveUndo?: boolean;

  /**
   * If `true`, `getValue('latex')` will return exactly the latest string passed
   * to `setValue()` until the user (or API commands) edits the content again.
   */
  faithfulLatex?: boolean;

  /**
   * If `true`, allow `#@` templates to capture a left-hand implicit argument.
   * Defaults to `false` so insertions stay scoped to the active selection/range.
   */
  captureImplicitArgument?: boolean;
...
/**
 * Cursor context returned by `mf.getEnvironmentContext()`.
 *
 * `environments` are ordered from nearest ancestor to farthest.
 *
 * @category Mathfield
 */
export type MathfieldEnvironmentContext = {
  /** Normalized offset used to resolve the context */
  offset: Offset;
  /** Parsing mode at the resolved offset */
  mode: ParseMode;
  /** Ancestor branch names (nearest first), e.g. `subscript`, `body` */
  branchPath: string[];
  /** Ancestor array-like environment names (nearest first) */
  environments: string[];
  /** Nearest array cell context, if the caret is inside a cell */
  nearestArray: {
