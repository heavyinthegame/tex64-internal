...
export class _Mathfield implements Mathfield, KeyboardDelegateInterface {
  ...
  // Exact LaTeX string passed to setValue() when faithful mode is enabled.
  // Cleared as soon as an editing command mutates the content.
  private faithfulLatexValue: string | null = null;
...
If you are using Vue, this may be because you are using the runtime-only build of Vue. Make sure to include \`runtimeCompiler: true\` in your Vue configuration. There may a warning from Vue in the log above.`,
  ...
  getEnvironmentContext(offset?: Offset): MathfieldEnvironmentContext {
    const lastOffset = this.model.lastOffset;
    ...
    const safeOffset =
      lastOffset >= 0
        ? this.model.normalizeOffset(requestedOffset)
        : 0;

    const atom = this.model.at(safeOffset);
    const environments: string[] = [];
    const branchPath: string[] = [];
    let nearestArray: MathfieldEnvironmentContext['nearestArray'] = null;

    let node: Atom | undefined = atom;
    while (node) {
      if (typeof node.parentBranch === 'string') {
        branchPath.push(node.parentBranch);
      }

      if (
        nearestArray === null &&
        node.parent?.type === 'array' &&
        Array.isArray(node.parentBranch) &&
        node.parentBranch.length === 2
      ) {
        const [row, column] = node.parentBranch;
        const arrayParent = node.parent as {
          environmentName?: string;
          isMultiline?: boolean;
        };
        nearestArray = {
          environmentName:
            typeof arrayParent.environmentName === 'string' &&
            arrayParent.environmentName.length > 0
              ? arrayParent.environmentName
              : 'array',
          row,
          column,
          isMultiline: Boolean(arrayParent.isMultiline),
        };
      }

      if (node.type === 'array') {
        const environmentName = (node as { environmentName?: string })
          .environmentName;
    ...
    return {
      offset: safeOffset,
      mode: getMode(this.model, safeOffset) ?? atom?.mode ?? 'math',
      branchPath,
      environments,
      nearestArray,
    };
  ...
  getValue(
    ...
    if (this.faithfulLatexValue !== null) {
      // Preserve exact round-trip for the common "whole field latex" reads.
      if (arg1 === undefined || arg1 === 'latex') return this.faithfulLatexValue;
    }
  ...
  setValue(value: string, options?: InsertOptions): void {
    ...
    const faithfulLatex = options.faithfulLatex === true;
    ...
    if (ModeEditor.insert(this.model, value, options)) {
      ...
      if (!couldUndo && !preserveUndo) this.undoManager.reset();
      if (preserveUndo) this.undoManager.stopCoalescing();
      this.undoManager.snapshot('set-value');
      this.faithfulLatexValue = faithfulLatex ? value : null;
    }
  }
