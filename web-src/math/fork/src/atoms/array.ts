...
export class ArrayAtom extends Atom {
  ...
  constructor(
    ...
    if (typeof options.environmentParameters === 'string')
      this.environmentParameters = options.environmentParameters;
  }
  ...
  toJson(): AtomJson {
    ...
    if (typeof this.environmentParameters === 'string')
      result.environmentParameters = this.environmentParameters;
  ...
  _serialize(options: ToLatexOptions): string {
    ...
    if (this.environmentName === 'lines' && this._rows.length > 1)
      ...
      result.push(
        `\\begin{${this.environmentName}}${this.environmentParameters ?? ''}`
      );

    if (this.environmentName === 'array' && !this.environmentParameters) {
      result.push('{');
