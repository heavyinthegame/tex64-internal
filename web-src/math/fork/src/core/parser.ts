...
export class Parser {
  ...
  scanColspec(): ColumnFormat[] | null {
    ...
    while (!this.end() && !(this.peek() === '<}>' || this.peek() === ']')) {
      ...
      if (literal === 'c' || literal === 'r' || literal === 'l')
      ...
      else if (literal === 'm' || literal === 'p' || literal === 'b') {
        if (this.match('<{>')) {
        ...
        if (this.match('<{>')) {
          ...
          const group = this.scan((token) => token === '<}>');
          this.endContext();
          if (literal === '@') result.push({ gap: group });
          if (!this.match('<}>')) this.onError({ code: 'unbalanced-braces' });
          continue;
        }
      } else if (literal === '<{>') {
        this.consumeBalancedUnknownArgument('<{>', '<}>');
      }
    }
  ...
  scanSymbolOrLiteral(): Atom[] | null {
    ...
    if (token.startsWith('\\')) {
      ...
      if (!def) {
        return [new UnknownCommandAtom(token)];
      }
  ...
  scanArgument(type: 'colspec'): null | ColspecArgument;
  ...
  scanArgument(type: ArgumentType): null | Argument {
    ...
    if (type === 'string') result = this.scanString();
      ...
      const colspecStart = this.index;
      const columns = this.scanColspec();
      result = columns
        ? {
            columns,
            raw: tokensToString(this.tokens.slice(colspecStart, this.index)),
          }
        : null;
    }
  ...
  private consumeBalancedUnknownArgument(open: Token, close: Token): boolean {
    if (!this.match(open)) return false;
    let depth = 1;
    ...
    if (depth !== 0) this.onError({ code: 'unbalanced-braces' });
    return true;
  }

  private scanUnknownCommandSuffix(): string {
    ...
    while (!this.end()) {
      ...
      if (this.consumeBalancedUnknownArgument('<{>', '<}>')) continue;
      if (this.consumeBalancedUnknownArgument('[', ']')) continue;
    ...
    return tokensToString(this.tokens.slice(start, this.index));
  }
  ...
  scanSymbolOrCommand(command: string): readonly Atom[] | null {
    ...
    if (!info) {
      ...
      const suffix = this.scanUnknownCommandSuffix();
      return [new UnknownCommandAtom(command, suffix)];
    }
