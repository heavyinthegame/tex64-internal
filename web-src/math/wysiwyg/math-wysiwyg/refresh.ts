...
import type { MathWysiwygConfig, SuggestOptions, TokenMatch } from "./types.js";

export type MathWysiwygRefreshOps = {
  refresh: (options?: SuggestOptions) => void;
  close: () => void;
  openExplicitSuggestions: () => boolean;
  updateConfig: (config: Partial<MathWysiwygConfig>) => void;
};

const isInSuppressedTextLiteralContext = (rawValue: string | null, cursorIndex: number) => {
...
const isInSuppressedTextContext = (mathfieldApi: any, cursorOffset: number) => {
  const mode = typeof mathfieldApi?.mode === "string" ? (mathfieldApi.mode as "math" | "text" | "latex") : null;
  if (mode === "text") {
    return true;
  }
  const modeAtCursor = getModeAtOffset(mathfieldApi, cursorOffset) ?? getModeAtOffset(mathfieldApi, cursorOffset - 1);
...
const findScopedSlashCommandMatch = (mathfieldApi: any, cursorOffset: number): TokenMatch | null => {
...
export const createMathWysiwygRefreshOps = (
  ...
  const refresh = (options: SuggestOptions = {}) => {
    try {
      ...
      const slashCommandMatch = findScopedSlashCommandMatch(mathfieldApi, cursorOffset) ?? toOffsetMatch(findSlashCommandToken(rawValue, cursorIndex));
      ...
      if (!options.explicit && wordMatch) {
        ...
        const minLength =
          wordMatch.kind === "command" ? AUTO_COMMAND_MIN_LENGTH : AUTO_WORD_ALLOWLIST.has(normalized) ? 2 : AUTO_WORD_MIN_LENGTH;
