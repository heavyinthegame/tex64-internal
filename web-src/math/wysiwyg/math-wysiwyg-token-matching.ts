...
export type TokenKind = "word" | "operator" | "command" | "slash-command";

export type TokenIndexMatch = {
  token: string;
  range: { start: number; end: number };
  kind: TokenKind;
};

export const isWordToken = (value: string) => /^(?=.*[A-Za-z])[A-Za-z0-9]+$/.test(value);
export const isCommandToken = (value: string) => /^[A-Za-z]+$/.test(value);
...
const AUTO_REPLACE_OPERATOR_CORRECTIONS: Array<{ token: string; suffix: string }> = [
  { token: "<=>", suffix: "\\leq>" },
  { token: "<->", suffix: "\\leftarrow>" },
];

export const findOperatorToken = (text: string, cursorIndex: number): TokenIndexMatch | null => {
...
export const findAutoReplaceCorrection = (
  text: string,
  cursorIndex: number
): TokenIndexMatch | null => {
...
export const findWordToken = (text: string, cursorIndex: number): TokenIndexMatch | null => {
...
export const findSlashCommandToken = (
  text: string,
  cursorIndex: number
): TokenIndexMatch | null => {
  let start = cursorIndex;
