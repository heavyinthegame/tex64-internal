...
const isWordToken = (value: string) => /^(?=.*[A-Za-z])[A-Za-z0-9]+$/.test(value);
const isCommandToken = (value: string) => /^[A-Za-z]+$/.test(value);
...
const findAutoReplaceCorrection = (text: string, cursorIndex: number): TokenIndexMatch | null => {
...
export const initMathWysiwyg = (deps: MathWysiwygDeps): MathWysiwygApi => {
  ...
  const buildMatrixOpCandidates = () => {
    ...
    const stack: Array<{ name: string; bodyStart: number }> = [];
  ...
  const syncMathfieldMode = (mathfieldApi: any, cursorOffset: number) => {
    ...
    const setMode = (nextMode: "math" | "text") => {
  ...
  const applyCandidate = (index: number) => {
    ...
    if (INTERTEXT_TEMPLATE_RE.test(insertedLatex)) {
      ...
      const targetOffset = Math.max(0, (position ?? 1) - 1);
    ...
    if (hasPlaceholderTemplate) {
      try {
        ...
        const target =
          ranges.find((range) => range.start >= insertionAnchorStart) ??
          ranges[0] ??
  ...
  const movePlaceholderByTab = (backward: boolean) => {
    if (!mathfield) {
      return false;
    }
    const mathfieldApi = mathfield as any;
