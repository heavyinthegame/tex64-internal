...
export const readSelectionRange = (
  mathfieldApi: MathFieldPlaceholderApi
): { start: number; end: number } | null => {
  const selection = mathfieldApi.selection as
    | [number, number]
...
const setSelectionCollapsed = (mathfieldApi: MathFieldPlaceholderApi, position: number) => {
...
const getInternalPlaceholderRanges = (mathfieldApi: MathFieldPlaceholderApi) => {
  ...
  const promptRanges: Array<{ start: number; end: number }> = [];
...
const getLiteralPlaceholderRanges = (mathfieldApi: MathFieldPlaceholderApi) => {
  ...
  let match: RegExpExecArray | null = PLACEHOLDER_TOKEN_REGEX.exec(latex);
  while (match) {
...
const getPromptRanges = (mathfieldApi: MathFieldPlaceholderApi) => {
  ...
  try {
    const ids = mathfieldApi.getPrompts() ?? [];
    const lastOffset =
      typeof mathfieldApi.lastOffset === "number" ? mathfieldApi.lastOffset : null;
    const seen = new Set<string>();
    const ranges: Array<{ start: number; end: number }> = [];
    for (const id of ids) {
  ...
  const snapshotPosition =
    typeof mathfieldApi.position === "number" ? mathfieldApi.position : null;
  const lastOffset =
    typeof mathfieldApi.lastOffset === "number" ? mathfieldApi.lastOffset : null;
  const ranges: Array<{ start: number; end: number }> = [];
...
export const createPlaceholderNavigator = () => {
  ...
  const moveMathFieldPlaceholder = (
    mathfieldHost: HTMLElement,
    direction: "forward" | "backward"
  ) => {
    const mathfieldApi = mathfieldHost as MathFieldPlaceholderApi;
    const selection = readSelectionRange(mathfieldApi);
    const selectionStart =
      selection?.start ??
      (typeof mathfieldApi.position === "number" ? mathfieldApi.position : 0);
    ...
    const isFullSelection = (range: { start: number; end: number } | null) => {
      if (!range) {
        return false;
      }
    ...
    const moveByCommandSkippingFull = (
      commandValue: string,
    ...
    const shouldUsePromptNavigation =
      promptRanges.length > 0 &&
    ...
    const moveWithinPrompts = () => {
      ...
      if (
        typeof lastPlaceholderIndex === "number" &&
      ...
      const findContaining = (pos: number) =>
        promptRanges.findIndex((range2) => pos >= range2.start && pos <= range2.end);
      let currentIndex = -1;
      if (range && !isFullSelection(range)) {
        currentIndex = promptRanges.findIndex(
    ...
    const charCommand =
      direction === "backward" ? "moveToPreviousChar" : "moveToNextChar";
    ...
    const command =
      direction === "backward" ? "moveToPreviousPlaceholder" : "moveToNextPlaceholder";
...
export const readMathFieldValue = (
...
export const writeMathFieldValue = (
